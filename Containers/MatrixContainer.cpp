 ...
 * Constructor.
 */
MatrixContainer::MatrixContainer() : mContainer()
{
} // end of Constructor.
...
void MatrixContainer::init()
{
  ...
  if (!params.getOnlyPostProcessingFlag())
  {
    ...
    mContainer[MI::kDdxKShiftPosR].set(
      MT::kComplex, DimensionSizes(reducedDims.nx, 1, 1), kLoad, kNoCheckpoint, kDdxKShiftPosRName);
    mContainer[MI::kDdyKShiftPos].set(
      MT::kComplex, DimensionSizes(1, reducedDims.ny, 1), kLoad, kNoCheckpoint, kDdyKShiftPosName);
    if (is3DSimulation)
    {
      mContainer[MI::kDdzKShiftPos].set(
        MT::kComplex, DimensionSizes(1, 1, reducedDims.nz), kLoad, kNoCheckpoint, kDdzKShiftPosName);
    }

    mContainer[MI::kDdxKShiftNegR].set(
      MT::kComplex, DimensionSizes(reducedDims.nx, 1, 1), kLoad, kNoCheckpoint, kDdxKShiftNegRName);
    mContainer[MI::kDdyKShiftNeg].set(
      MT::kComplex, DimensionSizes(1, reducedDims.ny, 1), kLoad, kNoCheckpoint, kDdyKShiftNegName);
    if (is3DSimulation)
    {
      mContainer[MI::kDdzKShiftNeg].set(
        MT::kComplex, DimensionSizes(1, 1, reducedDims.nz), kLoad, kNoCheckpoint, kDdzKShiftNegName);
    }
  ...
  {
    mContainer[MI::kSensorMaskIndex].set(
      MT::kIndex, DimensionSizes(params.getSensorMaskIndexSize(), 1, 1), kLoad, kNoCheckpoint, kSensorMaskIndexName);
  }

  // cuboid sensor mask
  if (params.getSensorMaskType() == Parameters::SensorMaskType::kCorners)
  {
    mContainer[MI::kSensorMaskCorners].set(MT::kIndex, DimensionSizes(6, params.getSensorMaskCornersSize(), 1), kLoad,
      kNoCheckpoint, kSensorMaskCornersName);
  }
  ...
  if (!params.getOnlyPostProcessingFlag())
  {
    ...
    {
      mContainer[MI::kInitialPressureSourceInput].set(
        MT::kReal, fullDims, kLoad, kNoCheckpoint, kInitialPressureSourceInputName);
    }

    // Velocity source index
    if ((params.getTransducerSourceFlag() != 0) || (params.getVelocityXSourceFlag() != 0) ||
        (params.getVelocityYSourceFlag() != 0) || (params.getVelocityZSourceFlag() != 0))
    {
      mContainer[MI::kVelocitySourceIndex].set(MT::kIndex, DimensionSizes(1, 1, params.getVelocitySourceIndexSize()),
    ...
    {
      mContainer[MI::kDelayMask].set(
        MT::kIndex, DimensionSizes(1, 1, params.getVelocitySourceIndexSize()), kLoad, kNoCheckpoint, kDelayMaskName);
    ...
    if (params.getPressureSourceFlag() != 0)
    {
      ...
      { // 1D case
        mContainer[MI::kPressureSourceInput].set(MT::kReal, DimensionSizes(1, 1, params.getPressureSourceFlag()), kLoad,
          kNoCheckpoint, kPressureSourceInputName);
      }
      else
      { // 2D case
        mContainer[MI::kPressureSourceInput].set(MT::kReal,
          DimensionSizes(1, params.getPressureSourceIndexSize(), params.getPressureSourceFlag()), kLoad, kNoCheckpoint,
          kPressureSourceInputName);
      }

      mContainer[MI::kPressureSourceIndex].set(MT::kIndex, DimensionSizes(1, 1, params.getPressureSourceIndexSize()),
    ...
    if (params.getVelocityXSourceFlag() != 0)
    {
      ...
      { // 1D
        mContainer[MI::kVelocityXSourceInput].set(MT::kReal, DimensionSizes(1, 1, params.getVelocityXSourceFlag()),
          kLoad, kNoCheckpoint, kVelocityXSourceInputName);
      }
      else
      { // 2D
        mContainer[MI::kVelocityXSourceInput].set(MT::kReal,
          DimensionSizes(1, params.getVelocitySourceIndexSize(), params.getVelocityXSourceFlag()), kLoad, kNoCheckpoint,
          kVelocityXSourceInputName);
      }
    } // ux_source_input
    ...
    if (params.getVelocityYSourceFlag() != 0)
    {
      if (params.getVelocitySourceMany() == 0)
      { // 1D
        mContainer[MI::kVelocityYSourceInput].set(MT::kReal, DimensionSizes(1, 1, params.getVelocityYSourceFlag()),
          kLoad, kNoCheckpoint, kVelocityYSourceInputName);
      }
      else
      { // 2D
        mContainer[MI::kVelocityYSourceInput].set(MT::kReal,
          DimensionSizes(1, params.getVelocitySourceIndexSize(), params.getVelocityYSourceFlag()), kLoad, kNoCheckpoint,
          kVelocityYSourceInputName);
      }
    } // uy_source_input
    ...
    if (is3DSimulation)
    {
      if (params.getVelocityZSourceFlag() != 0)
      {
        if (params.getVelocitySourceMany() == 0)
        { // 1D
          mContainer[MI::kVelocityZSourceInput].set(MT::kReal, DimensionSizes(1, 1, params.getVelocityZSourceFlag()),
            kLoad, kNoCheckpoint, kVelocityZSourceInputName);
        }
        else
        { // 2D
          mContainer[MI::kVelocityZSourceInput].set(MT::kReal,
            DimensionSizes(1, params.getVelocitySourceIndexSize(), params.getVelocityZSourceFlag()), kLoad,
            kNoCheckpoint, kVelocityZSourceInputName);
        }
      } // uz_source_input
    }
    ...
    if (((params.getVelocitySourceMode() == Parameters::SourceMode::kAdditive) ||
        ...
        (params.getPressureSourceFlag() || params.getVelocityXSourceFlag() || params.getVelocityYSourceFlag() ||
    ...
    if (params.getNonUniformGridFlag() != 0)
    {
      ...
      mContainer[MI::kDxudxnSgx].set(
        MT::kReal, DimensionSizes(fullDims.nx, 1, 1), kLoad, kNoCheckpoint, kDxudxnSgxName);
      mContainer[MI::kDyudynSgy].set(
        MT::kReal, DimensionSizes(1, fullDims.ny, 1), kLoad, kNoCheckpoint, kDyudynSgyName);
      if (is3DSimulation)
      {
        mContainer[MI::kDzudznSgz].set(
          MT::kReal, DimensionSizes(1, 1, fullDims.nz), kLoad, kNoCheckpoint, kDzudznSgzName);
      }
    }
  }
  ...
  if (params.getStoreVelocityNonStaggeredRawFlag() || params.getStoreVelocityNonStaggeredCFlag() ||
      params.getStoreIntensityAvgFlag() || params.getStoreQTermFlag() || params.getStoreIntensityAvgCFlag() ||
      params.getStoreQTermCFlag())
  {
    ...
    const size_t zCutSize = (is3DSimulation) ? fullDims.nx * fullDims.ny * nzR : 1;
