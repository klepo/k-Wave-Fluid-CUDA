...
void Parameters::init(int argc, char** argv) {
  ...
  if (getGitHash() != "") {
    Logger::log(Logger::LogLevel::kFull, kOutFmtGitHashLeft,
                getGitHash().c_str());
  ...
  if ((mCommandLineParameters.getSamplingStartTimeIndex() > mNt)
    ...
    throw std::invalid_argument(
      Logger::formatMessage(kErrFmtIllegalSamplingStartTimeStep, 1l, mNt));
  }

  // Too few steps to use overlapping compression
  if ((&CompressHelper::getInstance())->getPeriod()
      >= mNt - mCommandLineParameters.getSamplingStartTimeIndex()) {
...
void Parameters::printSimulatoinSetup() {
  Logger::log(Logger::LogLevel::kBasic, kOutFmtNumberOfThreads,
              getNumberOfThreads());

  Logger::log(Logger::LogLevel::kBasic, kOutFmtSimulationDetailsTitle);

  const string domainsSizes =
    (isSimulation3D()) ? Logger::formatMessage(
      kOutFmt3DDomainSizeFormat, getFullDimensionSizes().nx,
      getFullDimensionSizes().ny, getFullDimensionSizes().nz)
                       : Logger::formatMessage(kOutFmt2DDomainSizeFormat,
                                               getFullDimensionSizes().nx,
                                               getFullDimensionSizes().ny);

  // Print simulation size
  Logger::log(Logger::LogLevel::kBasic, kOutFmtDomainSize,
              domainsSizes.c_str());
...
void Parameters::readScalarsFromInputFile() {
  ...
  if (mFileHeader.getFileType() != Hdf5FileHeader::FileType::kInput) {
    throw ios::failure(Logger::formatMessage(kErrFmtBadInputFileFormat,
  ...
  if (!mFileHeader.checkMajorFileVersion()) {
    throw ios::failure(Logger::formatMessage(
      kErrFmtBadMajorFileVersion, getInputFileName().c_str(),
      mFileHeader.getFileMajorVersion().c_str()));
  }

  if (!mFileHeader.checkMinorFileVersion()) {
    throw ios::failure(Logger::formatMessage(
      kErrFmtBadMinorFileVersion, getInputFileName().c_str(),
  ...
  if (mFileHeader.getFileVersion() == Hdf5FileHeader::FileVersion::kVersion10) {
    mSensorMaskIndexSize =
      mInputFile.getDatasetSize(rootGroup, kSensorMaskIndexName);

    //if -u_non_staggered_raw enabled, throw an error - not supported
    if (getStoreVelocityNonStaggeredRawFlag()
        || getStoreVelocityNonStaggeredCFlag() || getStoreIntensityAvgFlag()
        || getStoreQTermFlag() || getStoreIntensityAvgCFlag()
        || getStoreQTermCFlag()) {
  ...
  if (mFileHeader.getFileVersion() == Hdf5FileHeader::FileVersion::kVersion11) {
    ...
    mInputFile.readScalarValue(rootGroup, kSensorMaskTypeName,
                               sensorMaskTypeNumericValue);
    ...
    switch (mSensorMaskType) {
      ...
      case SensorMaskType::kCorners: {
        ...
        mSensorMaskCornersSize =
          mInputFile
            .getDatasetDimensionSizes(rootGroup, kSensorMaskCornersName)
            .ny;
        break;
      }
    } // switch
  ...
  // flags
  mInputFile.readScalarValue(rootGroup, kPressureSourceFlagName,
                             mPressureSourceFlag);
  mInputFile.readScalarValue(rootGroup, kInitialPressureSourceFlagName,
                             mInitialPressureSourceFlag);

  mInputFile.readScalarValue(rootGroup, kTransducerSourceFlagName,
                             mTransducerSourceFlag);

  mInputFile.readScalarValue(rootGroup, kVelocityXSourceFlagName,
                             mVelocityXSourceFlag);
  mInputFile.readScalarValue(rootGroup, kVelocityYSourceFlagName,
                             mVelocityYSourceFlag);
  if (isSimulation3D()) {
    mInputFile.readScalarValue(rootGroup, kVelocityZSourceFlagName,
                               mVelocityZSourceFlag);
  }

  mInputFile.readScalarValue(rootGroup, kNonUniformGridFlagName,
                             mNonUniformGridFlag);
  ...
  if (mTransducerSourceFlag == 0) {
    ...
    mTransducerSourceInputSize =
      mInputFile.getDatasetSize(rootGroup, kTransducerSourceInputName);
  }
  ...
  if ((mTransducerSourceFlag > 0) || (mVelocityXSourceFlag > 0)
    ...
    mVelocitySourceIndexSize =
      mInputFile.getDatasetSize(rootGroup, kVelocitySourceIndexName);
  }

  // uxyz_source_flags,  in 2D mVelocityZSourceFlag is always 0
  if ((mVelocityXSourceFlag > 0) || (mVelocityYSourceFlag > 0)
      || (mVelocityZSourceFlag > 0)) {
    mInputFile.readScalarValue(rootGroup, kVelocitySourceManyName,
                               mVelocitySourceMany);

    size_t sourceModeNumericValue = 0;
    mInputFile.readScalarValue(rootGroup, kVelocitySourceModeName,
                               sourceModeNumericValue);
  ...
  if (mPressureSourceFlag != 0) {
    mInputFile.readScalarValue(rootGroup, kPressureSourceManyName,
                               mPressureSourceMany);

    size_t sourceModeNumericValue = 0;
    mInputFile.readScalarValue(rootGroup, kPressureSourceModeName,
                               sourceModeNumericValue);
    ...
    mPressureSourceIndexSize =
      mInputFile.getDatasetSize(rootGroup, kPressureSourceIndexName);
  } else {
    mPressureSourceMode = SourceMode::kDirichlet;
  ...
  if (mAbsorbingFlag != 0) {
    ...
    mAlphaCoeffScalarFlag =
      mInputFile.getDatasetDimensionSizes(rootGroup, kAlphaCoeffName)
      == scalarSizes;
  ...
  mC0ScalarFlag =
    mInputFile.getDatasetDimensionSizes(rootGroup, kC0Name) == scalarSizes;
  if (mC0ScalarFlag) {
    mInputFile.readScalarValue(rootGroup, kC0Name, mC0Scalar);
  }

  if (mNonLinearFlag) {
    mBOnAScalarFlag = mInputFile.getDatasetDimensionSizes(rootGroup, kBonAName)
                      == scalarSizes;
    if (mBOnAScalarFlag) {
  ...
  mRho0ScalarFlag =
    mInputFile.getDatasetDimensionSizes(rootGroup, kRho0Name) == scalarSizes;
  if (mRho0ScalarFlag) {
  ...
  if (mCommandLineParameters.getStorePressureCFlag()
    ...
    if (!mCommandLineParameters.getOnlyPostProcessingFlag()) {
      if (mCommandLineParameters.getPeriod() > 0
          && mCommandLineParameters.getFrequency() > 0) {
        throw ios::failure(
          Logger::formatMessage(kErrFmtBadPeriodAndFrequencyValue));
      }

      if (mCommandLineParameters.getFrequency() > 0) {
        try {
          mCommandLineParameters.mPeriod =
            1.0f / (mCommandLineParameters.getFrequency() * mDt);
          mInputFile.writeFloatAttribute(rootGroup, kPressureSourceInputName,
                                         Hdf5File::kPeriodName,
                                         mCommandLineParameters.getPeriod());
        } catch (...) {
          throw ios::failure(
            Logger::formatMessage(kErrFmtCannotComputePeriodValue));
        }
      }
        ...
        /*try
      {
      ...
      catch (...)*/
        {
          ...
          DimensionSizes size = mInputFile.getDatasetDimensionSizes(
            rootGroup, kPressureSourceInputName);
          hsize_t length = 0;
          hsize_t limit = 500;
          length = size.ny > limit ? limit : size.ny;
          float* data = static_cast<float*>(
            _mm_malloc(length * sizeof(float), kDataAlignment));
          // Read only part of the dataset
          hid_t dataset =
            mInputFile.openDataset(rootGroup, kPressureSourceInputName);
          mInputFile.readHyperSlab(
            dataset, DimensionSizes(size.nx / 2, size.ny - length, 0),
            DimensionSizes(1, length, 1), data);
          mInputFile.closeDataset(dataset);
          // Compute period
          mCommandLineParameters.mPeriod =
            CompressHelper::findPeriod(data, length);
          _mm_free(data);
          mInputFile.writeFloatAttribute(rootGroup, kPressureSourceInputName,
                                         Hdf5File::kPeriodName,
                                         mCommandLineParameters.getPeriod());
      ...
      if (!(mCommandLineParameters.getFrequency() > 0.0f)) {
        mCommandLineParameters.mFrequency =
          1.0f / (mCommandLineParameters.getPeriod() * mDt);
      }
    } else {
      try {
        Hdf5File outputFile;
        outputFile.open(mCommandLineParameters.getOutputFileName());
        std::string datasetName = (mSensorMaskType == SensorMaskType::kIndex)
                                    ? kPName + kCompressSuffix
                                    : kPName + kCompressSuffix + "/1";
        mCommandLineParameters.mPeriod = outputFile.readFloatAttribute(
          outputFile.getRootGroup(), datasetName, "c_period");
        mCommandLineParameters.mMOS = hsize_t(outputFile.readLongLongAttribute(
          outputFile.getRootGroup(), datasetName, "c_mos"));
        mCommandLineParameters.mHarmonics =
          hsize_t(outputFile.readLongLongAttribute(
            outputFile.getRootGroup(), datasetName, "c_harmonics"));
        mCommandLineParameters.mFrequency =
          1.0f / (mCommandLineParameters.getPeriod() * mDt);
        outputFile.close();
      } catch (std::exception&) {
        throw ios::failure(
          Logger::formatMessage(kErrFmtCannotReadCompressionAttributes));
      }
    }

    // Create compression helper
    CompressHelper& compressHelper = CompressHelper::getInstance();
    compressHelper.init(mCommandLineParameters.getPeriod(),
                        mCommandLineParameters.getMOS(),
                        mCommandLineParameters.getHarmonics(), true);
  }
...
void Parameters::saveScalarsToOutputFile() {
  ...
  mOutputFile.writeScalarValue(rootGroup, kPressureSourceFlagName,
                               mPressureSourceFlag);
  mOutputFile.writeScalarValue(rootGroup, kInitialPressureSourceFlagName,
                               mInitialPressureSourceFlag);

  mOutputFile.writeScalarValue(rootGroup, kTransducerSourceFlagName,
                               mTransducerSourceFlag);

  mOutputFile.writeScalarValue(rootGroup, kVelocityXSourceFlagName,
                               mVelocityXSourceFlag);
  mOutputFile.writeScalarValue(rootGroup, kVelocityYSourceFlagName,
                               mVelocityYSourceFlag);
  if (isSimulation3D()) {
    mOutputFile.writeScalarValue(rootGroup, kVelocityZSourceFlagName,
                                 mVelocityZSourceFlag);
  }

  mOutputFile.writeScalarValue(rootGroup, kNonUniformGridFlagName,
                               mNonUniformGridFlag);
  ...
  if ((mVelocityXSourceFlag > 0) || (mVelocityYSourceFlag > 0)
      || (mVelocityZSourceFlag > 0)) {
    mOutputFile.writeScalarValue(rootGroup, kVelocitySourceManyName,
                                 mVelocitySourceMany);
    mOutputFile.writeScalarValue(rootGroup, kVelocitySourceModeName,
                                 static_cast<size_t>(mVelocitySourceMode));
  }

  // p_source_flag
  if (mPressureSourceFlag != 0) {
    mOutputFile.writeScalarValue(rootGroup, kPressureSourceManyName,
                                 mPressureSourceMany);
    mOutputFile.writeScalarValue(rootGroup, kPressureSourceModeName,
                                 static_cast<size_t>(mPressureSourceMode));
  }
  ...
  if (getCopySensorMaskFlag()) {
    ...
    mOutputFile.writeScalarValue(rootGroup, kSensorMaskTypeName,
                                 sensorMaskTypeNumericValue);
  }
...
void Parameters::selectDevice() {
  ...
  Logger::log(Logger::LogLevel::kBasic, kOutFmtDeviceId,
              mCudaParameters.getDeviceIdx());

  Logger::log(Logger::LogLevel::kBasic, kOutFmtDeviceName,
              mCudaParameters.getDeviceName().c_str());
} // end of selectDevice
//----------------------------------------------------------------------------------------------------------------------
...
bool Parameters::isTimeToCheckpoint(TimeMeasure timer) const {
  ...
  const auto checkpointInterval =
    mCommandLineParameters.getCheckpointInterval();

  return (isCheckpointEnabled()
          && ((mTimeStepsToCheckpoint == 0)
              || ((checkpointInterval > 0)
                  && (timer.getElapsedTime() > float(checkpointInterval)))));
 ...
 */
Parameters::Parameters()
  : mCudaParameters(), mCommandLineParameters(), mInputFile(), mOutputFile(), mCheckpointFile(),
    mFileHeader(), mFullDimensionSizes(0, 0, 0),
    mReducedDimensionSizes(0, 0, 0), mNt(0), mTimeIndex(0),
    mTimeStepsToCheckpoint(std::numeric_limits<size_t>::max()), mDt(0.0f),
    mDx(0.0f), mDy(0.0f), mDz(0.0f), mCRef(0.0f), mC0ScalarFlag(false),
    mC0Scalar(0.0f), mRho0ScalarFlag(false), mRho0Scalar(0.0f),
    ...
    mAbsorbEtaScalar(0.0f), mAbsorbTauScalar(0.0f), mBOnAScalarFlag(false),
    mBOnAScalar(0.0f), mPmlXSize(0), mPmlYSize(0), mPmlZSize(0),
    mPmlXAlpha(0.0f), mPmlYAlpha(0.0f), mPmlZAlpha(0.0f),
    mPressureSourceFlag(0), mInitialPressureSourceFlag(0),
    mTransducerSourceFlag(0), mVelocityXSourceFlag(0),
    mVelocityYSourceFlag(0), mVelocityZSourceFlag(0),
    mPressureSourceIndexSize(0), mTransducerSourceInputSize(0),
    mVelocitySourceIndexSize(0), mPressureSourceMode(SourceMode::kDirichlet),
    mPressureSourceMany(0), mVelocitySourceMode(SourceMode::kDirichlet),
    mVelocitySourceMany(0), mSensorMaskType(SensorMaskType::kIndex),
    mSensorMaskIndexSize(0), mSensorMaskCornersSize(0) {
} // end of Parameters()
//----------------------------------------------------------------------------------------------------------------------
