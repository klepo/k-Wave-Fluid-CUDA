...
void CufftComplexMatrix::createR2CFftPlan1DX(const DimensionSizes& inMatrixDims) {
  ...
  // plan the FFT
  cufftError = cufftPlanMany(&sR2CFftPlan1DX, rank, n,
...
void CufftComplexMatrix::createR2CFftPlan1DY(const DimensionSizes& inMatrixDims) {
  ...
  cufftError = cufftPlanMany(&sR2CFftPlan1DY, rank, n,
                             inembed, istride, idist,
                             onembed, ostride, odist,
                             CUFFT_R2C, batch);
...
void CufftComplexMatrix::createR2CFftPlan1DZ(const DimensionSizes& inMatrixDims) {
  if (Parameters::getInstance().isSimulation2D()) {
    ...
    cufftError = cufftPlanMany(&sR2CFftPlan1DZ, rank, n,
                               inembed, istride, idist,
                               onembed, ostride, odist,
                               CUFFT_R2C, batch);
...
void CufftComplexMatrix::createC2RFftPlan1DX(const DimensionSizes& outMatrixDims) {
  ...
  cufftError = cufftPlanMany(&sC2RFftPlan1DX, rank, n,
                             inembed, istride, idist,
                             onembed, ostride, odist,
                             CUFFT_C2R, batch);
...
void CufftComplexMatrix::createC2RFftPlan1DY(const DimensionSizes& outMatrixDims) {
  ...
  cufftError = cufftPlanMany(&sC2RFftPlan1DY, rank, n,
                             inembed, istride, idist,
                             onembed, ostride, odist,
                             CUFFT_C2R, batch);
...
void CufftComplexMatrix::createC2RFftPlan1DZ(const DimensionSizes& outMatrixDims) {
  if (Parameters::getInstance().isSimulation2D()) {
    ...
    cufftError = cufftPlanMany(&sC2RFftPlan1DZ, rank, n,
                               inembed, istride, idist,
                               onembed, ostride, odist,
                               CUFFT_C2R, batch);
...
void CufftComplexMatrix::computeR2CFft1DY(RealMatrix& inMatrix) {
  ...
  SolverCudaKernels::trasposeReal3DMatrixXY<SolverCudaKernels::TransposePadding::kOutput>(mDeviceData,
...
void CufftComplexMatrix::computeR2CFft1DZ(RealMatrix& inMatrix) {
  if (Parameters::getInstance().isSimulation3D()) {
    ...
    SolverCudaKernels::trasposeReal3DMatrixXZ<SolverCudaKernels::TransposePadding::kOutput>(mDeviceData,
                                                                                            inMatrix.getDeviceData(),
                                                                                            dimSizes);
...
void CufftComplexMatrix::computeC2RFft1DY(RealMatrix& outMatrix) {
  ...
  SolverCudaKernels::trasposeReal3DMatrixXY<SolverCudaKernels::TransposePadding::kInput>(outMatrix.getDeviceData(),
                                                                                         mDeviceData,
                                                                                         dimSizes);
...
void CufftComplexMatrix::computeC2RFft1DZ(RealMatrix& outMatrix) {
  if (Parameters::getInstance().isSimulation3D()) {
    ...
    SolverCudaKernels::trasposeReal3DMatrixXZ<SolverCudaKernels::TransposePadding::kInput>(outMatrix.getDeviceData(),
                                                                                           getDeviceData(),
                                                                                           DimSizes);
  } else {
