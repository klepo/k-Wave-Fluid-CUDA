...
/*
 * Constructor of TMatrixContainer
 */
        ...
        switch (it->second.MatrixDataType){
            ...
            default:
            {
                PrintErrorAndThrowException(
                        MatrixContainer_ERR_FMT_RecordUnknownDistributionType,
                        it->second.HDF5MatrixName,
                        __FILE__,
                        __LINE__);
                break;
...
void TMatrixContainer::LoadDataFromInputHDF5File(THDF5_File & HDF5_File)
{
    for (TMatrixRecordContainer::iterator it = MatrixContainer.begin();
        ...
        if (it->second.LoadData) {
            it->second.MatrixPtr->
                ReadDataFromHDF5File(HDF5_File,
...
void TMatrixContainer::AddMatricesIntoContainer()
{
    ...
    MatrixContainer[kappa].SetAllValues(NULL,
    ...
    if (!Params->Get_c0_scalar_flag()) {
        MatrixContainer[c2].SetAllValues(NULL,
                                         TMatrixRecord::mdtReal,
                                         FullDims,
                                         LOAD,
                                         NOCHECKPOINT,
    ...
    MatrixContainer[p].SetAllValues(NULL,
                                    TMatrixRecord::mdtReal,
                                    FullDims,
                                    NOLOAD,
                                    CHECKPOINT,
    ...
    MatrixContainer[rhox].SetAllValues(NULL,
                                       TMatrixRecord::mdtReal,
                                       FullDims,
                                       NOLOAD,
                                       CHECKPOINT,
                                       rhox_Name);
    MatrixContainer[rhoy].SetAllValues(NULL,
                                       TMatrixRecord::mdtReal,
                                       FullDims,
                                       NOLOAD,
                                       CHECKPOINT,
    ...
    MatrixContainer[ddx_k_shift_pos].SetAllValues(NULL,
                                                  TMatrixRecord::mdtComplex,
                                                  TDimensionSizes(ReducedDims.X, 1, 1),
                                                  LOAD,
                                                  NOCHECKPOINT,
                                                  ddx_k_shift_pos_r_Name);
    MatrixContainer[ddy_k_shift_pos].SetAllValues(NULL,
                                                  TMatrixRecord::mdtComplex,
                                                  TDimensionSizes(1, ReducedDims.Y, 1),
                                                  LOAD,
                                                  NOCHECKPOINT,
                                                  ddy_k_shift_pos_Name);
    MatrixContainer[ddz_k_shift_pos].SetAllValues(NULL,
                                                  TMatrixRecord::mdtComplex,
                                                  TDimensionSizes(1, 1, ReducedDims.Z),
                                                  LOAD,
                                                  NOCHECKPOINT,
                                                  ddz_k_shift_pos_Name);

    MatrixContainer[ddx_k_shift_neg].SetAllValues(NULL,
                                                  TMatrixRecord::mdtComplex,
                                                  TDimensionSizes(ReducedDims.X ,1, 1),
                                                  LOAD,
                                                  NOCHECKPOINT,
    ...
    {
        MatrixContainer[sensor_mask_index].SetAllValues(
                NULL,
    ...
    {
        MatrixContainer[u_source_index].SetAllValues(
                NULL,
                TMatrixRecord::mdtIndex,
    ...
    if (Params->Get_transducer_source_flag() != 0)
    {
        MatrixContainer[delay_mask].SetAllValues(
                NULL,
                TMatrixRecord::mdtIndex,
                TDimensionSizes(1 ,1, Params->Get_u_source_index_size()),
                LOAD,
                NOCHECKPOINT,
                delay_mask_Name);
        MatrixContainer[transducer_source_input].SetAllValues(
                NULL,
                TMatrixRecord::mdtReal,
    ...
    if (Params->Get_p_source_flag() != 0) {
        if (Params->Get_p_source_many() == 0) { // 1D case
            MatrixContainer[p_source_input].SetAllValues(
                    NULL,
                    TMatrixRecord::mdtReal,
        ...
        MatrixContainer[p_source_index].SetAllValues(
                NULL,
                TMatrixRecord::mdtIndex,
    ...
    if (Params->Get_ux_source_flag() != 0) {
        if (Params->Get_u_source_many() == 0) { // 1D
            MatrixContainer[ux_source_input].SetAllValues(
                    NULL,
                    TMatrixRecord::mdtReal,
    ...
    if (Params->Get_uy_source_flag() != 0) {
        if (Params->Get_u_source_many() == 0) { // 1D
            MatrixContainer[uy_source_input].SetAllValues(
                    NULL,
                    TMatrixRecord::mdtReal,
                    TDimensionSizes(1, 1, Params->Get_uy_source_flag()),
                    LOAD,
                    NOCHECKPOINT,
                    uy_source_input_Name);
        }
        else { // 2D
            MatrixContainer[uy_source_input].SetAllValues(
                    NULL,
    ...
    if (Params->Get_uz_source_flag() != 0) {
        if (Params->Get_u_source_many() == 0) { // 1D
            MatrixContainer[uz_source_input].SetAllValues(
                    NULL,
                    TMatrixRecord::mdtReal,
                    TDimensionSizes(1, 1, Params->Get_uz_source_flag()),
                    LOAD,
                    NOCHECKPOINT,
                    uz_source_input_Name);
        }
        else { // 2D
            MatrixContainer[uz_source_input].SetAllValues(
                    NULL,
    ...
    MatrixContainer[CUFFT_X_temp].SetAllValues(NULL,
                                               ...
                                               NOCHECKPOINT,
                                               CUFFT_X_temp_Name);
    MatrixContainer[CUFFT_Y_temp].SetAllValues(NULL,
                                               TMatrixRecord::mdtCUFFT,
                                               ReducedDims,
                                               NOLOAD,
                                               NOCHECKPOINT,
                                               CUFFT_Y_temp_Name);
    MatrixContainer[CUFFT_Z_temp].SetAllValues(NULL,
                                               TMatrixRecord::mdtCUFFT,
                                               ReducedDims,
                                               NOLOAD,
                                               NOCHECKPOINT,
