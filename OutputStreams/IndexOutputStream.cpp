...
IndexOutputStream::IndexOutputStream(Hdf5File& file,
  ...
  : BaseOutputStream(file, datasetName, sourceMatrix, reduceOp, outputStreamContainer, doNotSaveFlag),
    mSensorMask(sensorMask), mDataset(H5I_BADID), mSampledTimeStep(0), mEventSamplingFinished()
{
...
void IndexOutputStream::create()
{
  ...
  mSize = (mReduceOp == ReduceOperator::kC)
            ? size_t(ceilf(mSensorMask.size() * mComplexSize)) * mCompressHelper->getHarmonics()
            : mSensorMask.size();
  ...
  if (!mDoNotSaveFlag)
  {
    ...
    {
      datasetSize = DimensionSizes(mSize, params.getNt() - params.getSamplingStartTimeIndex(), 1);
    }
    ...
    if (mReduceOp == ReduceOperator::kC)
    {
      mFile.writeLongLongAttribute(
        mFile.getRootGroup(), mRootObjectName, "c_harmonics", ssize_t(mCompressHelper->getHarmonics()));
...
void IndexOutputStream::reopen()
{
  ...
  else if (mReduceOp != ReduceOperator::kIAvg && mReduceOp != ReduceOperator::kQTerm &&
  ...
  { // aggregated quantities - reload data
    ...
    if (params.getTimeIndex() > params.getSamplingStartTimeIndex())
    {
      ...
      if (mReduceOp == ReduceOperator::kC)
      {
        mFile.readCompleteDataset(mFile.getRootGroup(), objectName, DimensionSizes(mOSize, 1, 1), mHostBuffer);
      }
      else
      {
        mFile.readCompleteDataset(mFile.getRootGroup(), objectName, DimensionSizes(mSize, 1, 1), mHostBuffer);
      }
      // Send data to device
      copyToDevice();
    }
...
void IndexOutputStream::sample()
{
  ...
  case ReduceOperator::kC:
  {
    OutputStreamsCudaKernels::sampleIndex<ReduceOperator::kNone>(
      mDeviceBuffer, mSourceMatrix.getDeviceData(), mSensorMask.getDeviceData(), mSensorMask.size());
...
#pragma omp for nowait
      for (size_t i = 0; i < mSensorMask.size(); i++)
      {
        // checkOrSetMinMaxValue(minValueLocal, maxValueLocal, sourceData[sensorData[i]], sensorMask.size() *
        // mSampledTimeStep + i);
