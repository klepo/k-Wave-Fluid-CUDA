...
void CuboidOutputStream::create() {
  ...
  mSize = (mReduceOp == ReduceOperator::kC) ? mSensorMask.getSizeOfAllCuboids(mCompressHelper->getHarmonics() * mComplexSize) : mSensorMask.getSizeOfAllCuboids();
...
void CuboidOutputStream::sample() {
  ...
  for (size_t cuboidIdx = 0; cuboidIdx < mCuboidsInfo.size(); cuboidIdx++) {
    ...
    const size_t nSamples = (mSensorMask.getBottomRightCorner(cuboidIdx) - mSensorMask.getTopLeftCorner(cuboidIdx)).nElements();
  ...
  if (mReduceOp == ReduceOperator::kNone) {
    ...
    /*for (size_t cuboidIdx = 0; cuboidIdx < mCuboidsInfo.size(); cuboidIdx++) {
      ...
      for (size_t i = cuboidStartIndex; i < cuboidStartIndex + cuboidSize.nElements(); i++) {
        checkOrSetMinMaxValue(minValueLocal, maxValueLocal, mHostBuffer[i], cuboidSize.nElements() * mSampledTimeStep + i);
      }
