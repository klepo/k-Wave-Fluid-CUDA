...
KSpaceFirstOrderSolver::KSpaceFirstOrderSolver()
  : mMatrixContainer(), mOutputStreamContainer(), mParameters(Parameters::getInstance()), mActPercent(0.0f),
    mIsTimestepRightAfterRestore(false), mTotalTime(), mPreProcessingTime(), mDataLoadTime(), mSimulationTime(),
...
void KSpaceFirstOrderSolver::compute()
{
  ...
  const string blockDims = Logger::formatMessage(
    kOutFmtCudaGridShapeFormat, cudaParameters.getSolverGridSize1D(), cudaParameters.getSolverBlockSize1D());

  Logger::log(Logger::LogLevel::kFull, kOutFmtCudaSolverGridShape, blockDims.c_str());

  const string gridDims = Logger::formatMessage(
    kOutFmtCudaGridShapeFormat, cudaParameters.getSamplerGridSize1D(), cudaParameters.getSamplerBlockSize1D());
  ...
  if (!mParameters.getOnlyPostProcessingFlag())
  {
    try
    {
      ...
      mNotSamplingTime =
        std::accumulate(mNotSamplingIterationTimes.begin(), mNotSamplingIterationTimes.end(), double(0));
...
// Compiler detections
  ...
  else
  {
    Logger::log(Logger::LogLevel::kBasic,
      ((cudaRuntimeVersion / 1000) < 10) ? kOutFmtCudaRuntime : kOutFmtCudaRuntime10, cudaRuntimeVersion / 1000,
      (cudaRuntimeVersion % 100) / 10);
  }

  int cudaDriverVersion;
  cudaDriverGetVersion(&cudaDriverVersion);
  Logger::log(Logger::LogLevel::kBasic, ((cudaDriverVersion / 1000) < 10) ? kOutFmtCudaDriver : kOutFmtCudaDriver10,
  ...
  {
    Logger::log(Logger::LogLevel::kBasic, kOutFmtCudaCodeArch, SolverCudaKernels::getCudaCodeVersion() / 10.f);
    Logger::log(Logger::LogLevel::kBasic, kOutFmtSeparator);
...
void KSpaceFirstOrderSolver::initializeCufftPlans()
{
  ...
  if (Parameters::getInstance().getStoreVelocityNonStaggeredRawFlag() ||
      ...
      Parameters::getInstance().getStoreIntensityAvgFlag() || Parameters::getInstance().getStoreQTermFlag() ||
      Parameters::getInstance().getStoreIntensityAvgCFlag() || Parameters::getInstance().getStoreQTermCFlag())
  {
    // X shifts
...
void KSpaceFirstOrderSolver::writeOutputDataInfo()
{
  ...
  mParameters.getOutputFile().writeStringAttribute(rootGroup, "/", "simulation_peak_host_memory_in_use",
    Logger::formatMessage("%ld MB", mSimulationPeakHostMemoryConsumption));
  mParameters.getOutputFile().writeStringAttribute(rootGroup, "/", "simulation_peak_device_memory_in_use",
    Logger::formatMessage("%ld MB", mSimulationPeakDeviceMemoryConsumption));
  mParameters.getOutputFile().writeStringAttribute(
    ...
    for (size_t i = 0; i < datasetSize; i += blockSize)
    {
      ...
      if (mParameters.getSensorMaskType() == Parameters::SensorMaskType::kIndex)
      {
        ...
        mParameters.getOutputFile().readHyperSlab(
          datasetP, DimensionSizes(i, 0, 0), datasetBlockSizes, dataP.getHostData());
        mParameters.getOutputFile().readHyperSlab(
          datasetUx, DimensionSizes(i, 0, 0), datasetBlockSizes, dataUx.getHostData());
        mParameters.getOutputFile().readHyperSlab(
          datasetUy, DimensionSizes(i, 0, 0), datasetBlockSizes, dataUy.getHostData());
        if (simulationDimension == SD::k3D)
        {
          mParameters.getOutputFile().readHyperSlab(
            datasetUz, DimensionSizes(i, 0, 0), datasetBlockSizes, dataUz.getHostData());
        }
      }
      else if (mParameters.getSensorMaskType() == Parameters::SensorMaskType::kCorners)
      {
        ...
        mParameters.getOutputFile().readHyperSlab(
          datasetP, DimensionSizes(0, 0, zOffset, 0), datasetBlockSizes, dataP.getHostData());
        mParameters.getOutputFile().readHyperSlab(
          datasetUx, DimensionSizes(0, 0, zOffset, 0), datasetBlockSizes, dataUx.getHostData());
        mParameters.getOutputFile().readHyperSlab(
          datasetUy, DimensionSizes(0, 0, zOffset, 0), datasetBlockSizes, dataUy.getHostData());
        if (simulationDimension == SD::k3D)
        {
          mParameters.getOutputFile().readHyperSlab(
            datasetUz, DimensionSizes(0, 0, zOffset, 0), datasetBlockSizes, dataUz.getHostData());
        }
      }
...
template<Parameters::SimulationDimension simulationDimension> void KSpaceFirstOrderSolver::computePressureNonlinear()
{
  ...
  { // absorbing case
    ...
    SolverCudaKernels::computePressureTermsNonlinear<simulationDimension>(
      densitySum, nonlinearTerm, velocityGradientSum, mMatrixContainer);
...
template<Parameters::SimulationDimension simulationDimension> void KSpaceFirstOrderSolver::computePressureLinear()
{
  ...
  { // absorbing case
    ...
    SolverCudaKernels::computePressureTermsLinear<simulationDimension>(
      densitySum, velocityGradientTerm, mMatrixContainer);
...
void KSpaceFirstOrderSolver::addVelocitySource()
{
  ...
  { // executed Dirichlet and AdditiveNoCorrection source
    ...
    {
      SolverCudaKernels::addVelocitySource(getUxSgx(), getVelocityXSourceInput(), getVelocitySourceIndex());
    }
    if (mParameters.getVelocityYSourceFlag() > timeIndex)
    {
      SolverCudaKernels::addVelocitySource(getUySgy(), getVelocityYSourceInput(), getVelocitySourceIndex());
    }

    if ((mParameters.isSimulation3D()) && (mParameters.getVelocityZSourceFlag() > timeIndex))
    {
      SolverCudaKernels::addVelocitySource(getUzSgz(), getVelocityZSourceInput(), getVelocitySourceIndex());
    }
  }
  else
  { // execute Additive source
    ...
    {
      scaleSource(
        scaledSource, getVelocityXSourceInput(), getVelocitySourceIndex(), mParameters.getVelocitySourceMany());
    ...
    {
      scaleSource(
        scaledSource, getVelocityYSourceInput(), getVelocitySourceIndex(), mParameters.getVelocitySourceMany());
    ...
    {
      scaleSource(
        scaledSource, getVelocityZSourceInput(), getVelocitySourceIndex(), mParameters.getVelocitySourceMany());
...
template<Parameters::SimulationDimension simulationDimension> void KSpaceFirstOrderSolver::addPressureSource()
{
  ...
  if (mParameters.getPressureSourceFlag() > timeIndex)
  {
    ...
    { // execute Additive source
      ...
      scaleSource(
        scaledSource, getPressureSourceInput(), getPressureSourceIndex(), mParameters.getPressureSourceMany());
 ...
 * Scale source signal
 */
void KSpaceFirstOrderSolver::scaleSource(
  RealMatrix& scaledSource, const RealMatrix& sourceInput, const IndexMatrix& sourceIndex, const size_t manyFlag)
{
  ...
  SolverCudaKernels::insertSourceIntoScalingMatrix(scaledSource, sourceInput, sourceIndex, manyFlag);
  // Compute FFT
