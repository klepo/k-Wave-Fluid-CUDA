 ...
 * Constructor.
 */
Hdf5FileHeader::Hdf5FileHeader() : mHeaderValues()
{
} // end of constructor
 ...
 * Copy constructor.
 */
Hdf5FileHeader::Hdf5FileHeader(const Hdf5FileHeader& src) : mHeaderValues(src.mHeaderValues)
{
...
void Hdf5FileHeader::writeHeaderToCheckpointFile(Hdf5File& checkpointFile)
{
  ...
  // Write header
  checkpointFile.writeStringAttribute(rootGroup, "/", sHeaderNames[FHI::kFileType], mHeaderValues[FHI::kFileType]);

  checkpointFile.writeStringAttribute(rootGroup, "/", sHeaderNames[FHI::kCreatedBy], mHeaderValues[FHI::kCreatedBy]);

  checkpointFile.writeStringAttribute(
    rootGroup, "/", sHeaderNames[FHI::kCreationDate], mHeaderValues[FHI::kCreationDate]);

  checkpointFile.writeStringAttribute(
    rootGroup, "/", sHeaderNames[FHI::kFileDescription], mHeaderValues[FHI::kFileDescription]);

  checkpointFile.writeStringAttribute(
    rootGroup, "/", sHeaderNames[FHI::kMajorVersion], mHeaderValues[FHI::kMajorVersion]);

  checkpointFile.writeStringAttribute(
    rootGroup, "/", sHeaderNames[FHI::kMinorVersion], mHeaderValues[FHI::kMinorVersion]);
 ...
 */
void Hdf5FileHeader::getExecutionTimes(
  double& totalTime, double& loadTime, double& preProcessingTime, double& simulationTime, double& postprocessingTime)
{
